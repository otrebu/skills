...
When wrapping up the session, propose closing the run's workers. Once the user agrees, close each ledger entry that Herdr still reports as the labelled worker this run created, and report anything you skipped. There is no agent-level close: close panes with `herdr pane close <pane_id>`, then close each tab this run created with `herdr tab close <tab_id>` once it is empty. Entries missing from the ledger stay open for the user to close.
