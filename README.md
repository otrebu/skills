...
| [`fable-herdr-orchestration`](./skills/fable-herdr-orchestration) | A Fable-specific binding of `herdr-orchestration` — requires a Fable control tower, binds the capability roles to Codex GPT-6 and Fable workers, and fixes the launch commands, escalating to Codex GPT-6 Astra for the hardest isolated work. Install alongside `herdr-orchestration`. |
